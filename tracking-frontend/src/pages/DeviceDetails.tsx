...
export default function DeviceDetails() {
  ...
  const getBatteryStatus = (voltage: number) => {
    if (voltage >= 12.5) return { text: "Excellent", color: "text-green-600", bg: "bg-green-100", percentage: 100 };
    if (voltage >= 12.2) return { text: "Good", color: "text-green-500", bg: "bg-green-50", percentage: 80 };
    if (voltage >= 11.8) return { text: "Fair", color: "text-yellow-600", bg: "bg-yellow-100", percentage: 60 };
    if (voltage >= 11.5) return { text: "Low", color: "text-orange-600", bg: "bg-orange-100", percentage: 40 };
    return { text: "Critical", color: "text-red-600", bg: "bg-red-100", percentage: 20 };
  };

  const getSignalStrength = (csq: number) => {
    if (csq >= 20) return { text: "Excellent", color: "text-green-600", bars: 5 };
    if (csq >= 15) return { text: "Good", color: "text-green-500", bars: 4 };
    if (csq >= 10) return { text: "Fair", color: "text-yellow-600", bars: 3 };
    if (csq >= 5) return { text: "Poor", color: "text-orange-600", bars: 2 };
    return { text: "Very Poor", color: "text-red-600", bars: 1 };
  };

  const getSatelliteStatus = (satellites: number) => {
    if (satellites >= 8) return { text: "Excellent", color: "text-green-600", percentage: 100 };
    if (satellites >= 6) return { text: "Good", color: "text-green-500", percentage: 75 };
    if (satellites >= 4) return { text: "Fair", color: "text-yellow-600", percentage: 50 };
    if (satellites >= 2) return { text: "Poor", color: "text-orange-600", percentage: 25 };
    return { text: "No Fix", color: "text-red-600", percentage: 0 };
  };
      ...
      <DashboardLayout>
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <p className="text-gray-500">No location data found for device {serial}</p>
          </div>
        </div>
      </DashboardLayout>
    );
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-1 font-mono">Serial: {serial}</p>
          </div>
          <div className="flex items-center space-x-4">
            ...
            <div className="flex items-center space-x-2 text-sm text-gray-500 px-3 py-2 bg-gray-100 rounded-lg">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="group relative overflow-hidden bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="relative p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs font-semibold text-blue-600 bg-blue-100 px-2 py-1 rounded-full">Real-time</span>
            </div>
            <p className="text-gray-500 text-sm font-medium mb-1">Speed</p>
            <p className="text-4xl font-bold text-gray-800">
              {location.speed || 0}
              <span className="text-lg font-medium text-gray-500 ml-1">km/h</span>
            </p>
        ...
        <div className="group relative overflow-hidden bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="relative p-6">
            ...
            <p className="text-gray-500 text-sm font-medium mb-1">Satellites</p>
            <p className="text-4xl font-bold text-gray-800">
              {location.sat || location.satellites || 0}
              <span className="text-lg font-medium text-gray-500 ml-1">in view</span>
            </p>
            <div className="mt-2">
        ...
        <div className="group relative overflow-hidden bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="relative p-6">
            ...
            <p className="text-gray-500 text-sm font-medium mb-1">Signal Strength</p>
            <p className="text-4xl font-bold text-gray-800">
              <span className={signalStrength.color}>{location.csq || 0}</span>
              <span className="text-lg font-medium text-gray-500 ml-1">/31</span>
            </p>
            <div className="mt-2 flex items-center">
        ...
        <div className="group relative overflow-hidden bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="relative p-6">
            ...
            <p className="text-gray-500 text-sm font-medium mb-1">Battery</p>
            <p className="text-4xl font-bold text-gray-800">
              <span className={batteryStatus.color}>{location.battery || 12}</span>
              <span className="text-lg font-medium text-gray-500 ml-1">volts</span>
            </p>
            <div className="mt-2">
              <div className="w-full bg-gray-200 rounded-full h-2">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              ...
              <span className="font-mono text-sm font-medium text-gray-800">{location.lat?.toFixed(6)}°</span>
            </div>
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              <span className="text-gray-600">Longitude</span>
              <span className="font-mono text-sm font-medium text-gray-800">{location.lng?.toFixed(6)}°</span>
            </div>
            {location.operator && (
              <div className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-gray-600">Network Operator</span>
                <span className="text-sm font-medium text-gray-800">{location.operator}</span>
              </div>
            )}
            {location.ignition !== undefined && (
              <div className="flex justify-between items-center py-2">
                <span className="text-gray-600">Ignition</span>
                <span className={`text-sm font-medium ${location.ignition ? 'text-green-600' : 'text-gray-500'}`}>
                  {location.ignition ? (
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
              ...
              <div className="flex items-center">
                <div className={`w-2 h-2 rounded-full mr-2 ${(location.sat || location.satellites || 0) >= 8 ? 'bg-green-500' : (location.sat || location.satellites || 0) >= 4 ? 'bg-yellow-500' : 'bg-red-500'}`}></div>
                <span className="text-sm font-medium">
                  {(location.sat || location.satellites || 0) >= 8 ? 'High' : (location.sat || location.satellites || 0) >= 4 ? 'Medium' : 'Low'}
                </span>
              </div>
            </div>
            <div className="flex justify-between items-center py-2 border-b border-gray-100">
